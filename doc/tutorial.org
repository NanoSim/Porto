...
it comes to production code we're ofte more interested in working with
compiled languanges such as C++, Fortran and Java. By bringing in the
same elements from Porto scripting into these languages we unleash the
true power of data-centric development. Being able to instanciate,
store and retrieve entites directly in the numerical simulators, we
reduce the unnecessary boilerplate code and IO infrastucture. Ideally
...
OO-antipatterns. However, this is the data-centric approach, and the
key strategy is to put the data up front.

In the back-end of our C++ framework we want to instanciate a Storage
...
instanciate a storage based on a mysql-database:
...
class. The main idea is that the entities themself are
...
points should be stored frequently to a local textfile, while the
...
int main(int, char**)
{
   ...
   Unit unit; // Instanciate our unit-entity
...
that actually feeds the storage datamodel with the contents of the
