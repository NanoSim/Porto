...
static bool writeAttributeBool (hid_t dataset, QString const &key, bool value)
{
  unsigned char uValue = (value ? 1 : 0);
  auto datatype = H5T_STD_U8LE;
  auto dataspace = H5Screate (H5S_SCALAR);
  auto attr = H5Acreate(dataset, qPrintable(key), datatype, dataspace, H5P_DEFAULT, H5P_DEFAULT);
  auto status = H5Awrite(attr, datatype, &uValue);
  H5Aclose(attr);
  H5Sclose(dataspace);
  return (status >= 0);
}
...
static bool writeAttribute (hid_t dataset, QString const &key, QVariant const &value)
{
  ...
  case QVariant::Bool:
    return writeAttributeBool (dataset, key, value.toBool());
  default:
    return false;
  }
}
...
static bool writeBool(hid_t file_id, QString const &path, bool value, QVariantMap const &attr)
{
  unsigned char v = (value ? 1 : 0);
  auto datatype  = H5T_STD_I8LE;
  auto dataspace = H5Screate(H5S_SCALAR);
  auto dataset   = H5Dcreate(file_id, qPrintable(path), datatype, dataspace, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  auto status    = H5Dwrite(dataset, datatype, H5S_ALL, H5S_ALL, H5P_DEFAULT, &v);
...
static QVariant::Type getListType(QVariant const &v)
{
  ...
  if (lst.count() > 0)
    ...
    return QVariant::List;  // indicates empty list
}
...
bool QH5 :: write (QString const &path, QVariant const &value, QVariantMap const &attributes)
{
  ...
  case QVariant::Bool:
    return writeBool(d->file->id(), path, value.toBool(), attributes);
