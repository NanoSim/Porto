...
namespace console {
  ...
  static QScriptValue raw (QScriptContext *context, QScriptEngine *engine)
  {
    if (context->argumentCount() == 0) {
      context->throwError("Missing argument");
      return engine->undefinedValue();
    }
    auto arg = context->argument(0);
    QTextStream(stdout) << arg.toString().toUtf8();
    return engine->undefinedValue();
  }  
  ...
  : QObject (engine)
{
  ...
  registerFunction(engine, "raw", console::raw, &console, "Print raw output to stdout, without newline.");
}
