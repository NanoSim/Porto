...
namespace filesystem {
  ...
  static QScriptValue filePath(QScriptContext *context, QScriptEngine *engine)
  {
    if (context->argumentCount() != 1) {
      context->throwError ("Syntax error");
      return engine->toScriptValue(false);
    }
    auto file = context->argument(0).toString();
    QFileInfo fileInfo(file);
    return engine->toScriptValue(fileInfo.path());
  }
...
{   
   ...
   registerFunction (engine, "filePath",       filesystem::filePath,       &fs, "Returns the path of the give filename");
